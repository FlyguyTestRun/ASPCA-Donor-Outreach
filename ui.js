...
(function () {
  ...
  var TOUR_STEPS = [
    {
      ...
      why: "The original asked one model to read a table, compute tiers, do the arithmetic, and write every letter in a single pass, with no way to check any step. Splitting the inputs you set from the math the code gets right the same way every time is the whole fix.",
    ...
    },
    {
      ...
      why: "references/policy.md requires a real signer, never invented. The original template just assumed a name would appear there.",
      prompt: null,
    },
    {
      ...
      why: "The original's worst line told the donor their gift &ldquo;will be matched, even if no match is confirmed.&rdquo; Here a match is only ever mentioned if you confirm one, with a real sponsor and terms.",
      prompt: null,
    },
    {
      sel: '[data-tour="donordata"]',
      audio: "walkthrough/step4.mp3",
      title: "Your data, and what it already caught",
      text: "The case study's 50 donors are loaded from a CSV file. Replace them with your own file or merge more in. The tool has already caught four donors whose stated tier did not match their giving history.",
      why: "The original hardcoded its donor table into its own instructions. Here the uploaded file is the only source of truth, and tier is recomputed from gift history rather than trusted from a label.",
      prompt: null,
    },
    {
      sel: '[data-tour="table"]',
      audio: "walkthrough/step5.mp3",
      title: "Review and resolve",
      text: "Edit any donor and everything recomputes live. Flagged accounts have hard stops. No letter generates until a person reviews that donor and confirms, so nothing is ever assumed.",
      why: "This replaces the original's &ldquo;make reasonable assumptions and proceed.&rdquo; Missing or ambiguous data routes to a human, every time.",
      prompt: null,
    },
    {
      sel: '[data-tour="export"]',
      audio: "walkthrough/step6.mp3",
      title: "The gate and the export",
      text: "Export stays locked until every exception is cleared and every flagged donor is confirmed. Once it is green, you get one zip: every letter, the manifest, the change log, and a working copy of this tool.",
      why: "It is the same package a fundraiser gets and the same one submitted for this case study. There is no separate version.",
      prompt: "That's the whole run. End the walkthrough whenever you're ready.",
    },
  ];
    var tourIndex = -1;
  function startTour() {
    ...
    ["tourNext", "tourBack", "tourEnd", "tourMute"].forEach(function (id) { document.getElementById(id).style.display = ""; });
    showTourStep();
  }
  ...
  function endTour() {
    ...
    ["tourNext", "tourBack", "tourEnd", "tourMute"].forEach(function (id) { document.getElementById(id).style.display = "none"; });
  ...
  var tourMuted = false;
  function updateTourAudio(step) {
    var a = document.getElementById("tourAudio");
  ...
  function tourToggleMute() {
    tourMuted = !tourMuted;
    var a = document.getElementById("tourAudio");
    var b = document.getElementById("tourMute");
    if (b) b.textContent = tourMuted ? "Unmute narration" : "Mute narration";
    if (tourMuted) { if (a) { try { a.pause(); } catch (e) {} } }
    else { tourReplayAudio(); }
  }
  ...
  window.UI = {
    ...
    tourStep: tourStep,
    endTour: endTour,
    tourReplayAudio: tourReplayAudio,
    tourToggleMute: tourToggleMute,
